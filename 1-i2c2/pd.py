...
class Decoder(srd.Decoder):
    ...
    options = (
        {'id': 'data_display', 'desc': 'Data display format',
            'default': 'hex', 'values': ('hex', 'ascii')},
    ...
    def handle_address_or_data(self, scl, sda):
        ...
        if not cmd.startswith('ADDRESS'):
            if self.options['data_display'] == 'hex':
                self.putx([proto[cmd][0], ['%s: %02X' % (proto[cmd][1], d),
                           '%s: %02X' % (proto[cmd][2], d), '%02X' % d]])
            else:
                self.putx([proto[cmd][0], ['%s: %s' % (proto[cmd][1], chr(d)),
                           '%s: %s' % (proto[cmd][2], chr(d)), '%s' % chr(d)]])
        else:
            self.putx([proto[cmd][0], ['%s: %02X' % (proto[cmd][1], d),
                       '%s: %02X' % (proto[cmd][2], d), '%02X' % d]])
